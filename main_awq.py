...
model_config = [{"model" : "meta-llama/Llama-3.1-8B-Instruct" ,"quantization": "AWQ", "load_format": None},
                {"model" : "meta-llama/Llama-3.2-3B-Instruct" ,"quantization": "AWQ", "load_format": None},
...
for model in model_config:
    ...
    tokens_out = [len(tokenizer.encode(text, add_special_tokens=False)) for text in responses]

    tokens_in = [len(tokenizer.encode(text, add_special_tokens=False)) for text in dataset["prompt"]]

    print(f"Total tokens generated by {model}:\n {tokens_out}")

    token_df.append({"model" : project_name, "tokens_out": tokens_out, "tokens_in":tokens_in})
